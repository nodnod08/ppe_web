	...
	<div class="container">
		...
		<div v-if="product != null && typeof product != 'undefined'" class="row">
			...
			<div class="col-lg-7">
				...
				<button type="button" @click="addToCart(product)" class="btn btn-sm btn-outline-secondary">
...
export default {
	...
	methods: {
		...
		addToCart: function(product) {
			...
			axios.post('/api-items/add-to-cart', { product }).then((res) => {
				let res1 = res.data.result;
				setTimeout(() => {
					this.$store.commit('modifyCart', {
						user: res1[0],
						items: JSON.parse(res1[1]),
					});
